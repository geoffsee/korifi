...
Pulumi/TypeScript installers that automate these guides live under [`deploy/`](./deploy) (`kind`, `k0s`, `eks`, `gke`), built from reusable components in [`deploy/lib`](./deploy/lib). For a local 3-node k0s cluster in Docker, see [`deploy/k0s`](./deploy/k0s).
