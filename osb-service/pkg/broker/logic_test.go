...
func TestBindRedisCredentials(t *testing.T) {
	b, err := NewBusinessLogic(Options{})
	if err != nil {
		t.Fatal(err)
	}
	id := "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee"
	_, status, err := b.Provision(id, ProvisionRequest{ServiceID: RedisServiceID, PlanID: RedisPlanID})
	if err != nil || status != http.StatusCreated {
		t.Fatalf("provision: %d %v", status, err)
	}
	resp, status, err := b.Bind(id, "bbbbbbbb-cccc-dddd-eeee-ffffffffffff", BindRequest{ServiceID: RedisServiceID, PlanID: RedisPlanID})
	if err != nil || status != http.StatusCreated {
		t.Fatalf("bind: %d %v", status, err)
	}
	...
	if !strings.HasPrefix(uri, "rediss://") {
		t.Fatalf("uri missing rediss scheme: %s", uri)
	}
}
